 ...
 */
async function uploadFotoCloudinary(base64: string, mimeType: string = 'image/jpeg'): Promise<string> {
    ...
    for (const foto of fotosRaw) {
      if (typeof foto === 'string' && foto.trim()) {
        ...
        if (typeof foto.url === 'string' && foto.url.trim()) {
          ...
          try {
            const cloudUrl = await uploadFotoCloudinary(foto.data, foto.mime);
