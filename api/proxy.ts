  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      switch (action) {
        ...
        case 'proxy':
          result = await uploadCloudinary(body.fileData || body.base64, body.mimeType || body.type);
          break;
        case 'uploadFoto':
          result = await uploadCloudinary(body?.data?.foto?.data || body.fileData || '', body?.data?.foto?.mime || body.mimeType);
