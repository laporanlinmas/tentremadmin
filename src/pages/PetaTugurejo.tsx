...
export const PetaTugurejo: React.FC = () => {
  ...
  const locateHandlersRef = useRef<{ found?: (event: any) => void; error?: () => void }>({});
    ...
    _ensureLeafletLoaded(() => {
      ...
      if (typeof ResizeObserver !== 'undefined') {
        resizeObserver = new ResizeObserver(() => {
          if (!disposed && mapRef.current === map) map.invalidateSize({ animate: false });
        });
        resizeObserver.observe(md);
      }
    });
  ...
  const handleLocateMe = () => {
    ...
    const previousHandlers = locateHandlersRef.current;
    if (previousHandlers.found) map.off('locationfound', previousHandlers.found);
    if (previousHandlers.error) map.off('locationerror', previousHandlers.error);
    ...
    locateHandlersRef.current = { found: handleLocationFound, error: handleLocationError };
