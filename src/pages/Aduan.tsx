import {
  ...
  Globe, Bot, Tag, User, Phone, PhoneCall, Plus, Trash2, Check, ExternalLink,
  ShieldCheck, Shield, Save, Eye, Layers, Sparkles, HeartPulse, Flame, Zap,
  LayoutList, LayoutGrid, X, Video
} from 'lucide-react';
...
const getKategoriBadge = (kat: string) => {
  ...
  if (k.includes('keamanan') || k.includes('polisi')) {
    return { bg: 'rgba(59, 130, 246, 0.12)', color: 'var(--blue)', border: '1px solid rgba(59, 130, 246, 0.3)' };
  }
  if (k.includes('medis') || k.includes('kesehatan') || k.includes('ambulans')) {
    return { bg: 'rgba(239, 68, 68, 0.12)', color: 'var(--red)', border: '1px solid rgba(239, 68, 68, 0.3)' };
  }
  if (k.includes('damkar') || k.includes('pemadam')) {
    return { bg: 'rgba(249, 115, 22, 0.12)', color: '#ea580c', border: '1px solid rgba(249, 115, 22, 0.3)' };
  }
  if (k.includes('linmas') || k.includes('posko') || k.includes('desa') || k.includes('aparat')) {
    return { bg: 'rgba(16, 185, 129, 0.12)', color: 'var(--teal)', border: '1px solid rgba(16, 185, 129, 0.3)' };
  }
  if (k.includes('bencana') || k.includes('bpbd') || k.includes('sar')) {
    return { bg: 'rgba(168, 85, 247, 0.12)', color: '#9333ea', border: '1px solid rgba(168, 85, 247, 0.3)' };
  }
  return { bg: 'rgba(14, 165, 233, 0.12)', color: '#0284c7', border: '1px solid rgba(14, 165, 233, 0.3)' };
};
